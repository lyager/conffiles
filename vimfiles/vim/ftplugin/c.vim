...
" DISABLED to do per-directory .vimrc files
" Below setings seems utopic
...
function! CscopeRefresh(...)
  if a:0 != 0
    :!rm -f cscope.files cscope.out<cr>
  end
  :CscopeBuild
  :cs reset
endfunction
...
function! CscopeBuild()
  ":exe "!rm -f cscope.files cscope.out"
  :exe "!find . -type f -o -type l | egrep -i '\\.([chly](xx|pp)*|cc|hh)$' > cscope.files\n"
  :exe "!cscope -b -i cscope.files"
  :exe "cs reset"
endfunction

command! -nargs=0 CscopeBuild :call CscopeBuild()
