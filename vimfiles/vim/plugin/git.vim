...
if !exists('g:git_no_map_default') || !g:git_no_map_default
    nnoremap <Leader>gd :GitDiff --relative --no-prefix<Enter>
    nnoremap <Leader>gD :GitDiff --relative --no-prefix 
    ...
    nnoremap <Leader>gw :GitWhatChanged<Enter>
endif
...
" Whatchanged
function! GitWhatChanged()
    " Wanna see progress...
    let git_output = s:SystemGit('whatchanged -n 1 -p ' . s:Expand('%'))
    if !strlen(git_output)
        echo "No output from git command"
        return
    endif
...
function! GitShowRev(args)
    let git_dir = <SID>GetGitDir()
    let file = s:Expand('%')
    let args = a:args

    let git_output = s:SystemGit('show ' . args . ':./'. file)
    call <SID>OpenGitBuffer(git_output)
endfunction
...
command! -nargs=1 GitShowRev          call GitShowRev(<q-args>)
...
command!          GitWhatChanged      call GitWhatChanged()
