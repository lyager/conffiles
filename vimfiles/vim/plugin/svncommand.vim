...
"
" SVNStatusAll     Performs "svn status -v" on all files within current
"                  directory.
"
...
" JLN
" This is the function I keep forgetting
" The function that actually gathers the 'svn' commandline command.
function! s:SVNCreateCommandBuffer(cmd, cmdName, statusText, origBuffNR)
  ...
  " JLN Added this, nice for debugging.
  echo "SVNCommand: ".svnCommand
  echo "\n"
...
" Function: s:SVNDoCommand(svncmd, cmdName, statusText, global) {{{2
" General skeleton for SVN function execution.
" Global indicates the now spefix filename should be used, it's handled for
"        now as an optional parametes
...
function! s:SVNDoCommand(cmd, cmdName, statusText, ...)
  ...
  let realFileName = ""
  if a:0 == 0 
    " If optional arguements are given it probably coz it's gloal
...
" Function: s:SVNCommitFile( filename ) {{{2
function! s:SVNCommitFile( filename )
  let svnBufferCheck=s:SVNCurrentBufferCheck()
  if svnBufferCheck ==  -1
    echo "Original buffer no longer exists, aborting."
    return -1
  ...
  "silent put='Filename: '.a:filename
  "silent put='realFilename: '.realFileName
  "silent put='realFilePath: '.realFilePath
...
function! s:SVNBlame()
  return s:SVNDoCommand('blame', 'svndiff', '')
endfunction

" Function: s:SVNStatusAll() {{{2
function! s:SVNStatusAll()
  return s:SVNDoCommand('status -v', 'svnstatus', '', 1)
endfunction
...
com! -nargs=1 SVNCommitFile call s:SVNCommitFile(<args>)
com! -nargs=0 SVNBlame call s:SVNBlame()
...
com! SVNStatusAll call s:SVNStatusAll()
...
nnoremap <silent> <Plug>SVNCommitFile :SVNCommitFile(resolve(expand('%')))<CR>
...
nnoremap <silent> <Plug>SVNStatusAll :SVNStatusAll<CR>
nnoremap <silent> <Plug>SVNBlame :SVNBlame<CR>
...
if !hasmapto('<Plug>SVNCommitFile')
  nmap <unique> <Leader>sC <Plug>SVNCommitFile
endif
...
if !hasmapto('<Plug>SVNStatusAll')
  nmap <unique> <Leader>sS <Plug>SVNStatusAll
endif
if !hasmapto('<Plug>SVNBlame')
  nmap <unique> <Leader>sb <Plug>SVNBlame
endif
...
amenu <silent> &Plugin.SVN.&StatusAll  <Plug>SVNStatusAll
amenu <silent> &Plugin.SVN.&Blame      <Plug>SVNBlame
